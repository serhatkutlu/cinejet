...
class GetHomeMoviesUseCase @Inject constructor(
    ...
    operator fun invoke(movieTypes: Map<MediaType, Int>): Flow<Resource<Map<MediaType, List<Movie>?>>> {
        val flows: List<Flow<Resource<List<Movie>>>> = movieTypes.map { (mediaType, page) ->
            repository.loadMoviesByMediaType(mediaType, page)
        }

        val mediaTypesList = movieTypes.keys.toList()
        ...
        return combine(flows) { resources: Array<Resource<List<Movie>>> ->
            when {
                ...
                resources.any { it is Resource.Error<*> } -> {
                    ...
                    val mapResult = mediaTypesList.zip(resources).associate { (mediaType, resource) ->
                    ...
                    val mapResult =
                        mediaTypesList.zip(resources).associate { (mediaType, resource) ->
